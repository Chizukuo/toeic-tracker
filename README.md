...
> **"20 Days to Mastery."** 
> 纯本地、高颜值的 TOEIC 冲刺训练看板。集成计时、复盘、生词、估分、同步，全流程闭环，助力 20 天高效提分。
...
- **线性冲刺流**：将计时、对分、错题归因整合成无缝的单向工作流，消除决策损耗。
- **生词记忆仓库**：不再只是记录，通过自动词典查阅、例句提取与重复频率追踪，构建属于你的考点词库。
- **PEASEA 估分体系**：基于非线性插值与异常响应惩罚，提供比简单换算表更具参考价值的预测。
- **隐私与自由**：数据 100% 存在本地，不收录隐私，不依赖服务器，随时一键导出与跨端同步。
...
```
/              Mission Control — 总览仪表盘、倒计时、活动地图
/plan          Sprint Plan — 20 日练习计划与状态切换
